<script lang="ts">
    ...
    const oversizeMessage = $derived(
        policy?.display?.oversizeMessage ??
            "File too large for Plotly time-series preview (max 25 MiB)"
    );
    ...
    const plotlyChart: Action<HTMLDivElement, PlotlyActionParams> = (node, params) => {
        ...
        const loadAndRender = async (nextParams: PlotlyActionParams) => {
            ...
            try {
                ...
                if (!cancelled && runId === currentRun) {
                    plotError =
                        error instanceof Error ? error.message : "Failed to render Plotly graph";
                    renderedPlot = false;
                }
            }
        };
...
{:then plotData}
    <div class="space-y-3">
        <div class="grid grid-cols-1 gap-2 text-xs text-gray-600 sm:grid-cols-3">
            <div>
                <span class="font-medium text-gray-800">Time field:</span>
                {plotData.timeField}
            </div>
