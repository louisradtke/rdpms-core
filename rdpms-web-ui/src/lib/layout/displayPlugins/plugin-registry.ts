...
const corePluginPolicies: Partial<Record<CorePluginId, CorePluginPolicy>> = {
	[CORE_PLUGIN_IDS.table]: {
		download: {
			...
			description: 'Table previews download the full file into the browser before rendering it.',
...
export function autoPluginCandidates(file: FileSummaryDTO): CorePluginId[] {
	...
	if (mime === 'text/csv' || mime === 'application/csv' || ['csv', 'tsv'].includes(abbreviation)) {
