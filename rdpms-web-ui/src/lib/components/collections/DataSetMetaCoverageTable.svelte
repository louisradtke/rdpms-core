<script lang="ts">
    ...
    let metadataEditorDatasetId = $state('');
    ...
    const openMetadataEditor = (
        ...
        if (!datasetId || !metadataKey) return;
        metadataEditorOpen = true;
        metadataEditorPending = false;
        metadataEditorError = '';
        metadataEditorJson = '{\n  \n}';
        metadataEditorTitle = `${datasetName} / ${metadataKey}`;
        metadataEditorKey = metadataKey;
        metadataEditorDatasetId = datasetId;
        metadataEditorSchemaDbId = schemaDbId ?? '';
        metadataEditorSchemaId = schemaId ?? '';
    };

    const submitMetadataEditor = async () => {
        if (!metadataEditorDatasetId || !metadataEditorKey) {
            metadataEditorError = 'Missing target dataset or metadata key.';
            return;
        }
        ...
        try {
            const repo = new MetaDataRepository(getOrFetchConfig().then(toApiConfig));
            await repo.setDatasetMetadata(metadataEditorDatasetId, metadataEditorKey, metadataEditorJson);
            closeMetadataEditor();
            onDataChanged();
        } catch (err) {
