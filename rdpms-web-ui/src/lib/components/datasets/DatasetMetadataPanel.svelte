<script lang="ts">
    ...
    const datasetColumns = $derived(
        (columns ?? []).filter(
            (column: MetaDateCollectionColumnDTO) =>
                column.target === MetadataColumnTargetDTO.Dataset
        )
    );
    ...
    const effectiveColumns = $derived.by(() => {
        ...
        return (dataset.metaDates ?? []).map(
            (meta: AssignedMetaDateDTO) =>
                ({
    ...
    const findAssigned = (column: MetaDateCollectionColumnDTO) =>
        dataset.metaDates?.find(
            (entry: AssignedMetaDateDTO) =>
                normalizeKey(entry.metadataKey) === normalizeKey(column.metadataKey)
        );
