<script lang="ts">
	...
	const getVisualizationView = async (
		...
		const isValidated =
			metadata.validatedSchemas?.some((schema) => isVisualizationSchema(schema.schemaId)) ?? false;
...
<Sidebar itemsPromise={allDataSets} baseUrl="/projects/{projectId}/c/{collectionId}/*" />
