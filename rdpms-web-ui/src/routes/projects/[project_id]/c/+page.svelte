<script lang="ts">
    ...
    let tableAccessors: Array<{
        visibleTitle: string;
        accessor: (collection: CollectionSummaryDTO) => string;
    }> = [
        { visibleTitle: "ID", accessor: (collection) => collection.id ?? "null" },
        { visibleTitle: "Name", accessor: (collection) => collection.name ?? "null" },
        {
            visibleTitle: "# Data Sets",
            accessor: (collection) => collection.dataSetCount?.toString() ?? "-1"
        },
        {
            visibleTitle: "Default Data Store",
            accessor: (collection) => collection.defaultDataStoreId ?? "null"
        }
    ];
</script>
            ...
            <tbody>
                {#each collections as collection (collection.id)}
                    <tr class="even:bg-gray-50">
                        ...
                        {#each tableAccessors
                            .slice(1)
                            .map((dct) => dct.accessor(collection)) as entry (entry)}
