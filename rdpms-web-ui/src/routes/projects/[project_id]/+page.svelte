<script lang="ts">
    ...
    function sortCollections(
        collections: CollectionSummaryDTO[] | null | undefined
    ): CollectionSummaryDTO[] {
    ...
    async function openCreate(project: ProjectSummaryDTO) {
        ...
        creatingForProject = {
            id: currentProjectId,
            name: project.name ?? project.slug ?? currentProjectId
        };
        collectionForm = {
            name: "",
            slug: "",
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <section class="mt-2 lg:col-span-2">
                <div class="mb-3 flex items-center justify-between">
                    ...
                    <button
                        class="rounded-md bg-gray-800 px-3 py-1.5 text-sm text-white hover:bg-black/90"
                ...
                <ul class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {#each sortCollections(project.collections) as collection (collection.id)}
                        <li class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <a
                                        href="/projects/{projectId}/c/{collection.slug ??
                                            collection.id}"
                                ...
                                <span
                                    class="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-700"
                                >
                                    {collection.dataSetCount +
                                        " dataset" +
                                        (collection.dataSetCount === 1 ? "" : "s")}
                                </span>
                            </div>
                            <p class="mt-3 text-sm text-gray-600">
                                Default store: <span class="font-medium text-gray-800"
                                    >{defaultStoreName(collection, project.dataStores)}</span
                                >
                            </p>
                    ...
                    {:else}
                        <p class="text-sm text-gray-600">Project contains no collections</p>
                    {/each}
                </ul>
            ...
            <aside class="lg:col-span-1 lg:sticky lg:top-4 h-fit">
                ...
                <div class="space-y-4">
                    {#each sortStores(project.dataStores) as store (store.id)}
                        ...
                        <article class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                            <header class="mb-3 flex items-start justify-between gap-3">
                                ...
                                <span
                                    class="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-xs uppercase text-gray-700"
                                >
                            ...
                            <div class="mb-3 flex items-center justify-between gap-3">
                                ...
                                <button
                                    ...
                                    disabled={!store.id}
                                    aria-label={store.id
                                        ? `Copy store id for ${storeDisplayName(store)}`
                                        : "Store id unavailable"}
                                ...
                                >
                                    ...
                                    >
                                        <path
                                            d="M8 2a2 2 0 0 0-2 2v1H5a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h5a2 2 0 0 0 2-2v-1h1a2 2 0 0 0 2-2V6.5L11.5 2H8Zm0 1.5h3V5a1 1 0 0 0 1 1h1.5V9a.5.5 0 0 1-.5.5H12V8a2 2 0 0 0-2-2H7.5V4a.5.5 0 0 1 .5-.5Zm-3 4h5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-.5.5H5a.5.5 0 0 1-.5-.5v-7A.5.5 0 0 1 5 7.5Z"
                                        />
                                    </svg>
                            ...
                            <dl class="space-y-2 text-sm text-gray-700">
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    <dd class="font-medium text-gray-900">
                                        {formatBytes(store.storageBytes)}
                                    </dd>
                                </div>
                                <div class="flex items-center justify-between gap-3">
                                    <dt>References</dt>
                                    <dd class="text-gray-900">
                                        {formatReferenceCount(
                                            store.storageReferenceCount ?? store.filesCount
                                        )}
                                    </dd>
                                </div>
                                <div class="flex items-center justify-between gap-3">
                                    <dt>Access</dt>
                                    <dd class="text-gray-900">
                                        {store.canWrite ? "writable" : "read-only"}
                                    </dd>
                                </div>
                                {#if properties.endpointUrl}
                                    <div class="flex min-w-0 items-center justify-between gap-3">
                                        ...
                                        <dd
                                            class="min-w-0 truncate font-mono text-gray-900"
                                            title={properties.endpointUrl}
                                        >
                                ...
                                {#if properties.keyPrefix !== undefined}
                                    <div class="flex min-w-0 items-center justify-between gap-3">
                                        ...
                                        <dd
                                            class="min-w-0 truncate font-mono text-gray-900"
                                            title={properties.keyPrefix || "(root)"}
                                        >
                    ...
                    {:else}
                        <p
                            class="rounded-lg border border-dashed border-gray-300 p-4 text-sm text-gray-600"
                        >
            ...
            <div class="w-full max-w-2xl rounded-lg bg-white shadow-lg">
                ...
                >
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            <label class="block text-sm font-medium mb-1" for="collection-name"
                                >Name</label
                            >
                            <input
                                id="collection-name"
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-1" for="collection-slug"
                                >Slug</label
                            >
                            <input
                                id="collection-slug"
                        ...
                        <div class="md:col-span-2">
                            <label
                                ...
                                for="collection-description">Description</label
                            >
                            <textarea
                        ...
                        <div class="md:col-span-2">
                            <label
                                class="block text-sm font-medium mb-1"
                                for="collection-default-store">Default Data Store</label
                            >
                            {#if storesLoading}
                            ...
                            {:else}
                                ...
                                >
                                    ...
                                    {#each writableStores as store (store.id)}
                                        <option value={store.id ?? ""}>
                                            {store.name ?? store.slug ?? store.id} ({store.slug ??
                                                "no-slug"})
                                        </option>
                                    {/each}
                                </select>
                                {#if writableStores.length === 0}
                                    <p class="mt-1 text-sm text-amber-700">
                                        No writable stores found for this project.
                                    </p>
                                {/if}
                            {/if}
                        </div>
