...
| Time Series (Plotly) | `rdpms.timeseries-plotly` | `TimeSeriesPlotlyPlugin.svelte` | CSV-like inputs that are marked as time series                                                 | Renders a Plotly chart. Supports `renderer.options` such as `timeField`, `series`, `mode`, `title`, `xAxisTitle`, and `yAxisTitle`. Preview limit is 25 MiB. |
...
- For CSV/TSV data, the candidate order is `rdpms.timeseries-plotly`, `rdpms.table`, then `rdpms.code` when the file is marked as a time series. Otherwise it remains `rdpms.table`, then `rdpms.code`.
...
- Visualization manifest `renderer.options` are forwarded into `FileDisplay.svelte` and can be consumed by built-in plugins.
...
- `rdpms-web-ui/src/lib/layout/displayPlugins/TimeSeriesPlotlyPlugin.svelte`
